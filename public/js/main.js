...
function fill_eventos(conds){
  ...
  var regex = /(\?)?(offset=[^&]+)?(&amp;tipo=[^&]+)?(&amp;centros=[^&]+)?(&amp;fecha=.+)?/;
	...
	for(i = 0; i < parseInt(result["pages"]); i++){
		pag_html = pag_html + '<li' + (parseInt(result["curr_page"]) == i - 1 ? ' class="active"' : '') + '><a data-remote="true" href="/eventos?offset=' + (i-1).toString() + ($("#tipo").val() != "" ? "&conds[tipo]=" + $("#tipo").val() : "") + ($("#centros").val() != "" ? "&conds[centros]=" + $("#centros").val() : "") + ($("#fecha").val() != "" ? "&conds[fecha]=" + $("#fecha").val() : "") + '">' + (i + 1).toString() + '</a></li>';
	}
