function fill_videoteca(conds){
  ...
  var regex = /(\?)?(offset=.+)?(&autor=.+)?(&pais=.+)?(&fecha=.+)?(&tema=.+)?/;
	...
	var offset = (grupos !== undefined && grupos[1] !== undefined ? "&offset=" + parseInt(grupos[2].split("=")[1]) : "");
	...
	var pag_html = '<li><a data-remote="true" href="/videoteca?offset=' + (result["prev_page"] == null ? '0' : result["prev_page"]) + '"><i class="fa fa-angle-left"></i><span class="sr-only">Anterior</span></a></li>';
	...
	pag_html = pag_html + '<li><a data-remote="true" href="/videoteca?offset=' + (result["next_page"] == null ? '0' : result["next_page"]) + '"><i class="fa fa-angle-right"></i><span class="Siguiente</span></a></li>';
	$("#pags").html(pag_html);
