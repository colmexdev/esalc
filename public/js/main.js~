function fill_videoteca(conds){
  ...
  var regex = /(\?)?(offset=[^&]+)?(&amp;tema=[^&]+)?(&amp;autor=[^&]+)?(&amp;pais=[^&]+)?(&amp;fecha=.+)?/;
	...
	var autor = (grupos !== undefined && grupos[4] !== undefined ? "&clave=" + grupos[4].split("=")[1] : "");
	...
	$.ajax({
		url: "https://coed.colmex.mx/catalogo_videos.json?tags=Latinoamerica"+tema+"&crono=desc&vpp=10" + offset + autor + fecha,
			...
			for(var i = 0; i < result["vids"].length; i++){
				...
				vids_html = vids_html + '<dt>' +
                        ...
                        '<dd>' +
                          '<div class="row">' +
                            '<div class="col-md-7">' +
                              '<p class="text-left"><strong class="azul-obscuro">Descripción:</strong> <span>' + result["vids"][i]["descripcion"] + '</span></p>' +
...
function fill_eventos(conds){
  ...
  var regex = /(\?)?(offset=[^&]+)?(&amp;tipo=[^&]+)?(&amp;centros=[^&]+)?(&amp;fecha=.+)?/;
	...
	var fecha = (grupos !== undefined && grupos[6] !== undefined ? "&anio=" + grupos[6].split("=")[1] : "");
	$.ajax({
		url: "https://www.colmex.mx/catalogo_eventos.json?categoria=1:Estudios sobre América Latina" + "&crono=desc&limit=10" + offset + centros + tipo + fecha,
		success: function(result){
			var vids_html = "";
			for(var i = 0; i < result["videos"].length; i++){
				//var fecha_vid = result["videos"][i]["fecha"].substring(0,10).split("-");
				vids_html = vids_html + '<dt>' +
                    '<a href="#"><h5 class="m-0 text-left">' + result["videos"][i]["tituloActividad"] + '</h5><h5 class="verde-obscuro mt-10 mb-0">' + result["videos"][i]["subtituloActividad"] + '</h5></a>' +
                  '</dt>' +
                  '<dd>' +
                    '<div class="row">' +
                      '<div class="col-md-7">' +
                      ...
                      '</div>' +
                      '<div class="col-md-12">' +
                        '<p class="text-left"><strong class="azul-obscuro">Enlace al evento:</strong> <a href="' + result["videos"][i]["liga"] + '" target="_blank" class="a-inv">' + result["videos"][i]["liga"] + '</a></p>' +
                        '<p class="text-left"><strong class="azul-obscuro">Programa:</strong> <a href="http://www.h-mexico.unam.mx/node/17683" target="_blank" class="a-inv">http://www.h-mexico.unam.mx/node/17683</a></p>' +
                        //'<p class="text-left"><strong class="azul-obscuro">Videos relacionados:</strong> <a href="#" target="_blank" class="a-inv">Crisis, política y gestión migratoria con Demetrios Papademetriou</a>,
                          //<a href="#" target="_blank" class="a-inv">Efectos del control migratorio en el sitio de trabajo con Pia Orrenius</a>, <a href="#" target="_blank" class="a-inv">El rol del Estado en la gestión migratoria con James Hollifield</a>,
                          //<a href="#" target="_blank" class="a-inv">Migración y desarrollo con Hein de Haas</a></p>
                      '</div>' +
                    '</div>' +
                  '</dd>';
			}
	...
	var pag_html = '<li><a data-remote="true" href="/eventos?offset=' + (result["prev_page"] == null ? '0' : result["prev_page"]) + ($("#tipo").val() != "" ? "&conds[tipo]=" + ($("#tipo").val() != "" ? "&conds[tipo]=" + $("#tipo").val() : "") + ($("#centros").val() != "" ? "&conds[centros]=" + $("#centros").val() : "") + ($("#fecha").val() != "" ? "&conds[fecha]=" + $("#fecha").val() : "") + '"><i class="fa fa-angle-left"></i><span class="sr-only">Anterior</span></a></li>';
