class PrincipalController < ApplicationController
  def principal
    @pubs = Publicacion.order(anio: :desc, updated_at: :desc).limit(4)
		@pubs_hm = Publicacion.where("titulo like 'Historia Mínima%' and titulo not like '%M_xico%'").order(anio: :desc, updated_at: :desc).limit(4)
  end

  def libros
    @pubs_r = Publicacion.order(anio: :desc, updated_at: :desc).limit(4)
		@pubs_hm = Publicacion.where("lower(titulo) like 'historia mínima%' and titulo not like '%M_xico%'").order(anio: :desc, updated_at: :desc).limit(4)
    where = ""
    if params.key?(:conds)
  ...
  def historia
    ...
    @revs = Revista.where(where + (params.key?(:conds) && params[:conds].key?(:key) ? (" AND (articulo like '%" + params[:conds][:key] +"%' OR autores like '%" + params[:conds][:key] + "%' OR revista like '%" + params[:conds][:key] + "%')") : "")).order(created_at: :asc).as_json.each {|m| m[:sect] = "rev" }
  ...
  def articulos
    ...
    @mags = Revista.where(where).order(created_at: :asc).offset(params.key?(:offset) ? (params[:offset].to_i * limite) : 0).limit(limite)
